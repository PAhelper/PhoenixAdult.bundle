...
def search(results,encodedTitle,title,searchTitle,siteNum,lang,searchByDateActor,searchDate,searchSiteID):
    ...
    if apiKEY:
        for sceneType in ['scenes', 'movies']:
            ...
            prefix = 'all_'
            if PAsearchSites.getSearchSiteName(siteNum) == 'Girlfriends Films':
                prefix = 'girlfriendsfilms_'
            indexName = prefix + sceneType
            data = getAlgolia(url, indexName, params, PAsearchSites.getSearchBaseURL(siteNum))
    ...
    if PAsearchSites.getSearchSiteName(siteNum) == 'Girlfriends Films':
        ...
        for searchResult in searchResults.xpath('//div[@class="grid-item"]'):
            ...
            results.Append(MetadataSearchResult(id='%s|%d' % (curID, siteNum), name='[GFF.net] %s' % (titleNoFormatting), score=score, lang=lang))
...
def update(metadata,siteID,movieGenres,movieActors):
    ...
    if sceneType:
        ...
        prefix = 'all_'
        if PAsearchSites.getSearchSiteName(siteID) == 'Girlfriends Films':
            prefix = 'girlfriendsfilms_'
        indexName = prefix + sceneType

        url = PAsearchSites.getSearchSearchURL(siteID) + '?x-algolia-application-id=TSMKFA364Q&x-algolia-api-key=' + apiKEY
        data = getAlgolia(url, indexName, 'filters=%s=%d' % (sceneIDName, sceneID), PAsearchSites.getSearchBaseURL(siteID))
        detailsPageElements = data['results'][0]['hits'][0]

        data = getAlgolia(url, prefix + 'scenes', 'query=%s' % detailsPageElements['url_title'], PAsearchSites.getSearchBaseURL(siteID))['results'][0]['hits']
        ...
        for collectionName in ['network_name', 'serie_name', 'movie_title']:
            if collectionName in detailsPageElements:
                metadata.collections.add(detailsPageElements[collectionName].split(':')[0])
        ...
        for actorLink in actors:
            ...
            data = getAlgolia(url, prefix + 'actors', 'filters=actor_id=' + actorLink['actor_id'], PAsearchSites.getSearchBaseURL(siteID))
        ...
        if 'pictures' in detailsPageElements:
            ...
            for idx, scene in scenesPagesElements:
                keys = [key for key in scene['pictures'].keys() if key[0].isdigit()]
                max_quality = sorted(keys)[-1]
                art.append('https://images-fame.gammacdn.com/movies/' + scene['pictures'][max_quality])

    elif PAsearchSites.getSearchSiteName(siteID) == 'Girlfriends Films':
