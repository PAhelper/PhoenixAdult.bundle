...
def search(results,encodedTitle,title,searchTitle,siteNum,lang,searchByDateActor,searchDate,searchSiteID):
    ...
    titleNoFormatting = searchResult.xpath('//p[@class="video-title"]')[0].text_content().strip()
    curID = url.replace('/','_').replace('?','!')
    subSite = "Foster Tapes"
    if searchDate:
...
def update(metadata,siteID,movieGenres,movieActors):
    ...
    # Title
    metadata.title = detailsPageElements.xpath('//p[@class="video-title"]')[0].text_content().strip()
    ...
    actors = detailsPageElements.xpath('//p[@class="model-name"]/a')
    if len(actors) > 0:
