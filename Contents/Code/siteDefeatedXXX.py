...
def search(results,encodedTitle,title,searchTitle,siteNum,lang,searchDate):
    ...
    for searchResult in searchResults.xpath('//h2[contains(@class,"post-title") and contains(@class,"entry-title")]'):
        ...
        curID = String.Encode(searchResult.xpath('//h2[contains(@class,"post-title") and contains(@class,"entry-title")]/a')[i].get('href'))
        Log("curID: " +curID)
        try:
...
def update(metadata,siteID,movieGenres,movieActors):
    ...
    # TITLE
    metadata.title = detailsPageElements.xpath('//meta[@property="og:title"]')[0].get('content').replace('| Defeatedxxx', '')
    ...
    posters = detailsPageElements.xpath('//img[(contains(@class,"alignnone size-full ")) and not(contains(@class,"wp-image-4512"))]')
